 ...
 * product composite (dashboard screenshot + "Untitled Database" code card) is
 * moved to the RIGHT half as a floating, bleeding-off-edge visual.
 *
 * Left text column aligns to the page content-left (x=80 @1440, matching the
 * kit Container). Background image and product assets are unchanged Sasonix
 * placeholders (tokens.ts) pending the CaseDelta rebrand.
 *
...
const DASH_W = 860;
const DASH_TOP = 262;
const DASH_RIGHT = -70; // bleed a touch off the right edge
const CODE_W = 372;
const CODE_TOP = 452;
const CODE_LEFT = 686; // floats over the dashboard's lower-left, bridging toward the text
...
export function Hero() {
  return (
    <section style={{ position: "relative", height: HERO_H, background: SX.white, fontFamily: SX.body, overflow: "hidden" }}>
      ...
      {/* left-darkening scrim so the white text reads over the bright sky */}
      <div aria-hidden style={{ position: "absolute", inset: 0, zIndex: 1, background: "linear-gradient(to right, rgba(6,6,10,0.62) 0%, rgba(6,6,10,0.42) 26%, rgba(6,6,10,0.12) 48%, rgba(6,6,10,0) 66%)" }} />
      {/* bottom fade into the page (the Framer "Linear" scrim) */}
      <div aria-hidden style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: 220, zIndex: 2, background: `linear-gradient(to bottom, rgba(255,255,255,0) 0%, rgba(255,255,255,0.85) 62%, ${SX.white} 100%)` }} />
      ...
      <img
        ...
        style={{ position: "absolute", top: DASH_TOP, right: DASH_RIGHT, width: DASH_W, height: "auto", zIndex: 5, borderRadius: 14, border: "1px solid rgba(255,255,255,0.55)", boxShadow: "0 34px 90px rgba(8,10,20,0.30)" }}
      />
      <CodeCard />

      {/* left text column (aligned to the page content-left) */}
      <div style={{ position: "relative", zIndex: 10, maxWidth: 1360, margin: "0 auto", padding: "0 40px", height: "100%" }}>
        <div style={{ paddingTop: 196, maxWidth: 588 }}>
          ...
          <h1 style={{ fontFamily: SX.display, fontWeight: 500, fontSize: 64, lineHeight: "68px", letterSpacing: "-2.4px", color: "#fff", margin: 0, maxWidth: 560 }}>
            Run more cases without hiring
          </h1>

          <p style={{ fontFamily: SX.body, fontWeight: 400, fontSize: 18, lineHeight: "30px", color: "rgba(255,255,255,0.88)", margin: "22px 0 0", maxWidth: 472 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 44, textShadow: "0 1px 18px rgba(0,0,0,0.35)" }}>
...
/* The "Untitled Database" card: a composite of the frame PNG (white body + footer),
   ...
   Repositioned to float over the right-half dashboard. */
function CodeCard() {
  const CARD_H = Math.round((CODE_W * 1200) / 1848); // frame aspect 1848x1200
  return (
    <div style={{ position: "absolute", top: CODE_TOP, left: CODE_LEFT, width: CODE_W, height: CARD_H, zIndex: 6, borderRadius: 12, boxShadow: "0 24px 80px rgba(8,10,20,0.30)", overflow: "hidden", background: "#fff" }}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
