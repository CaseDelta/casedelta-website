      ...
      <Container>
        ...
        <div style={{ position: "relative", width: "100%", height: 605, borderRadius: 24, overflow: "hidden" }}>
          ...
          <svg viewBox="0 0 1280 605" fill="none" aria-hidden style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} preserveAspectRatio="none">
            <g stroke="#ffffff" strokeWidth="1.6" opacity="0.9" strokeLinecap="round">
          ...
          {NODES.map((n, i) => (
            // eslint-disable-next-line @next/next/no-img-element
            <img key={i} src={n.src} alt="" aria-hidden style={{ position: "absolute", left: n.x, top: n.y, width: n.w, height: "auto", filter: "drop-shadow(0 10px 24px rgba(0,0,0,0.16))" }} />
          ))}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginTop: 40 }}>
          {TEXTS.map((t) => (
            <div key={t} style={{ background: SX.cream, borderRadius: 12, border: "1px solid rgba(26, 23, 18, 0.09)", padding: "32px 32px 35px" }}>
