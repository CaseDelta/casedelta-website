...
> **Status notes:**
> - **AgentFrame is on hold** as of March 2026. All AgentFrame references in this document are preserved for future reference but are not current priorities. See CLAUDE.md.
> - **The proactive agent loop has been re-prioritized to immediate** (pulled forward from "not built" to highest near-term priority) per `CASEDELTA_NARRATIVE.md`. The behavioral model — Delta working proactively in the background — is now core product, not a future feature.
> - **See also:** `CASEDELTA_NARRATIVE.md` (behavioral north star), `CASEDELTA_CONSOLIDATED.md` (strategy anchor).
...
- **Use model routing.** Route high-complexity tasks (100+ page contracts, multi-document synthesis) to Claude Opus with extended thinking (50,000 thinking tokens). Route routine tasks (classification, simple extraction) to Haiku or open-source models. The config-driven model already supports this via the `model_id` field. *(Note: "AgentFrame config-driven model" referenced in prior versions — AgentFrame is on hold; this capability lives in CaseDelta's own Strands config.)*
...
The gap between proprietary and open-source agentic capabilities has narrowed dramatically. This validates a BYOM (bring-your-own-model) strategy where CaseDelta's orchestration layer becomes more valuable as a model-agnostic platform. *(Note: Original text referenced "AgentFrame" here — AgentFrame is on hold; this capability is part of CaseDelta's core architecture.)*
...
Solves the observability gap ("Fragmented — CloudWatch + custom"). Deploy on ECS and instrument every agent invocation for per-tenant trace visibility — the "unified tracing, evals, error recovery" differentiator for CaseDelta's agent runtime. *(AgentFrame on hold — this is a CaseDelta infrastructure investment.)*
...
**Appsmith** (Apache 2.0, 35,000+ stars): Drag-and-drop admin UI builder for the CaseDelta admin dashboard. Ship an MVP dashboard in days instead of building React from scratch. *(Originally noted as "AgentFrame dashboard" — AgentFrame is on hold; applies to CaseDelta's own admin surface.)*

**Redoc** (MIT): Generates 3-panel API documentation from OpenAPI specs — for any future developer-facing API portal.

**OpenAPI Generator**: Produces Python and TypeScript SDKs automatically from API definitions — applicable if CaseDelta opens a public API.
...
**BYOM for CaseDelta's model layer.** As open-source models become agentic-capable, CaseDelta's model-agnostic orchestration layer becomes more valuable — letting the firm choose any model (open or closed) while getting multi-tenancy, governance, and compliance. The more models exist, the more valuable a model-agnostic platform becomes. *(AgentFrame on hold — this is a CaseDelta architecture strength, not a separate product.)*
...
> **Priority re-order (March 2026):** Items 1–3 below are superseded by the behavioral north star in `CASEDELTA_NARRATIVE.md`. The proactive loop (Clio + morning briefing cron + Firm context file + approval queue UI) is the highest-ROI investment available. Original items 1–8 are preserved for reference but should be sequenced after the proactive loop is live.

**Re-prioritized (do first — weeks 1–2):**
- **Finish Clio integration** (2–3 days) — the foundational connection that enables the proactive loop. Everything else depends on this.
- **Build Firm CLAUDE.md system** (1–2 days) — per-firm context file loaded into every Delta agent invocation. Defines practice areas, document types, attorney preferences. The institutional memory moat starts here.
- **Wire morning briefing cron** (2–3 days) — EventBridge fires at 7 AM per firm, Strands agent runs across all active cases, posts findings to approval queue and sends briefing.
- **Approval queue UI MVP** (3–4 days) — the primary interface: cards of Delta's pending actions with [Approve] [Edit] [Dismiss]. This replaces the blank chat input as the entry experience.

**Original items (sequence after proactive loop is live):**
1. Evaluate Odoo CRM's community edition — spin up a Docker instance and map its data model against CaseDelta's client/matter structure.
2. Deploy Presidio as a Lambda layer for document preprocessing. Low-effort, high-impact for the security narrative.
3. Test PaddleOCR v3.0 on actual legal documents (contracts, financial disclosures, immigration forms). Compare accuracy and speed against current pipeline.
4. Set up Langfuse on ECS and instrument Strands agent invocations. Observability foundation for CaseDelta's agent runtime. *(AgentFrame on hold.)*
5. Implement DSPy-based prompt optimization for Strands reflection prompts. Benchmark against current agent performance on legal document analysis.
6. Test Qwen3-Coder on Bedrock as an alternative model for routine document classification. Compare cost and quality against current Llama 3.3 default.
7. Begin legal model fine-tuning project — curate 5,000 legal document examples, use Claude as teacher, fine-tune Phi-4 as student.
8. Integrate pgvector into Supabase for RAG, reducing dependency on shared Bedrock Knowledge Base.
